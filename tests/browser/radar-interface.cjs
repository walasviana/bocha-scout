...
(async()=>{const browser=await chromium.launch({headless:true,channel:'chrome'});try{
 ...
 const result=await p.evaluate(async()=>{
  ...
  const missing=radarData([{name:'X',color:'Vermelho',plays:[{play:'Batida',result:'Erro'}]}],['Batida','Aérea']);
 ...
 await p.getByRole('button',{name:'End 2',exact:true}).click();assert.ok((await p.locator('#radar-test').innerText()).includes('Nenhum fundamento registrado'));assert.equal(await p.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);assert.deepEqual(errors,[]);await context.close();
