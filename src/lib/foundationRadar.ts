...
export function drawRadar(canvas:HTMLCanvasElement,series:RadarSeries[],axes=usedFoundations(series)){
 ...
 series.forEach((s,k)=>{const values=rows.map(row=>row.values[k]);ctx.strokeStyle=radarColor(s.color);ctx.fillStyle=radarColor(s.color);ctx.lineWidth=4;ctx.setLineDash([]);
