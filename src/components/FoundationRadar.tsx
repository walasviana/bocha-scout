...
export default function FoundationRadar({series,gameType,selectedEnd,hideFilters=false}:Props){
 ...
 <div className="radar-series-list">{visible.map((s,i)=>{const total=rows.reduce((sum,row)=>sum+row.values[i].total,0);return <div className="radar-series-card" key={`${s.name}-${i}`} style={{borderTopColor:radarColor(s.color)}}><strong style={{color:radarColor(s.color)}}>{s.name}</strong><span>{total} {total===1?'jogada':'jogadas'} · {i?'Linha tracejada':'Linha contínua'}</span></div>;})}</div>
 {axes.length>0?<><div className="radar-chart-surface"><canvas ref={canvas} role="img" aria-label={`${axes.length<3?'Comparação em barras':'Radar de fundamentos'}, ${end}, escala de 0 a 100%. Valores disponíveis em Detalhes do gráfico.`}/></div>
 ...
 <details className="radar-details"><summary>Detalhes do gráfico <span>Valores e quantidade de jogadas</span></summary><div className="radar-values">{rows.map(row=><div key={row.name}><strong>{row.name}</strong>{row.values.map((v,i)=><span key={i} style={{color:radarColor(series[i].color)}}>{series[i].name}: <b>{v.value===null?'sem dados':`${v.value.toFixed(1)}%`}</b>{v.value!==null&&` · ${v.total} ${v.total===1?'jogada':'jogadas'}`}</span>)}</div>)}</div></details></>:<div className="radar-empty"><strong>Nenhum fundamento registrado</strong><p>Os fundamentos aparecerão aqui conforme as jogadas forem registradas. Você também pode consultar outra parcial ou período.</p></div>}
