...
export default function FoundationRadar({series,gameType,selectedEnd,hideFilters=false}:Props){
 ...
 <div className="radar-series-list">{visible.map((s,i)=>{const total=rows.reduce((sum,row)=>sum+row.values[i].total,0);const isYellow=String(s.color||'').toLowerCase()==='amarelo';const textColor=isYellow?'#b45309':radarColor(s.color);return <div className="radar-series-card" key={`${s.name}-${i}`} style={{borderTopColor:radarColor(s.color)}}><strong style={{color:textColor}}>{s.name}</strong><span>{total} {total===1?'jogada':'jogadas'} · {i?'Pontos vazados':'Pontos preenchidos'}</span></div>;})}</div>
 ...
 <div className="radar-accessible-values" aria-label="Dados do gráfico">{rows.map(row=><div key={row.name}><strong>{row.name}</strong>{row.values.map((v,i)=>{const isY=String(series[i]?.color||'').toLowerCase()==='amarelo';const textColor=isY?'#b45309':radarColor(series[i]?.color);return <span key={i} style={{color:textColor}}>{series[i].name}: <b>{v.value===null?'sem dados':`${v.value.toFixed(1)}%`}</b>{v.value!==null&&` · ${v.total} ${v.total===1?'jogada':'jogadas'}`}</span>;})}</div>)}</div></>:<div className="radar-empty"><strong>Nenhum fundamento registrado</strong><p>Os fundamentos aparecerão aqui conforme as jogadas forem registradas. Você também pode consultar outra parcial ou período.</p></div>}
