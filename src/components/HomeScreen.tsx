...
export function HomeHeader({notifications,account}:{notifications:React.ReactNode;account:React.ReactNode}){
 ...
 return <header ref={headerRef} onFocusCapture={()=>setHidden(false)} className={`account-toolbar hub-modern-header${hidden?' is-retracted':''}${scrolled?' is-scrolled':''}`}><div className="home-brand"><img src="/bocha-scout-emblem.png" alt=""/><span>BOCHA <b>SCOUT</b></span></div><div className="home-header-actions">{notifications}{account}</div></header>;
}
