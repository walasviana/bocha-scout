...
export default function AccountNotifications({userId,role}: {userId:string;role:string}) {
 ...
 async function load() {
   const [n,r,c]=await Promise.all([
     ...
     supabase.from('notification_reads').select('notification_id,status').eq('user_id',userId),
     supabase.rpc('account_notification_count')
   ]);
   if(n.error || r.error || c.error) {
 ...
 async function decide(n:any,status:string) {
   ...
   const calls:Record<string,[string,any]>={
     ...
     scout:['admin_set_scout_approval',{target_session_id:n.source_id,new_status:status}],
     athlete_edit:['resolve_athlete_correction',{notification_id:n.id,new_status:status}]
   };
   const call=calls[n.source_type];
   if(!call){
     setError('Tipo de pedido desconhecido.');
     setBusy(false);
     return;
   }
   const {error}=await supabase.rpc(call[0],call[1]);
 ...
 const modal=open ? createPortal(
   <div
     className="account-notifications-overlay"
     role="dialog"
     aria-modal="true"
     aria-label="Notificações"
     onMouseDown={(event)=>{if(event.target===event.currentTarget)setOpen(false);}}
     style={{background:'rgba(15,23,42,.7)',overflowY:'auto',padding:18,color:'#0f172a'}}
   >
     <div style={{maxWidth:720,margin:'20px auto',background:'#fff',padding:22,borderRadius:16}}>
       <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center'}}>
         <h2 style={{margin:0}}>Notificações</h2>
         <button style={btn} onClick={()=>setOpen(false)}>Fechar</button>
       </div>
       {error && <p role="alert" style={{color:'#b91c1c'}}>{error}</p>}
       <button style={{...btn,marginTop:14}} disabled={busy} onClick={load}>Atualizar</button>
       {!visible.length && <p>Nenhuma notificação para esta conta.</p>}
       {visible.map(n=><article key={n.id} style={{padding:'16px 0',borderBottom:'1px solid #e2e8f0'}}>
 ...
 return <>
   <button
     ...
     title={error || 'Notificações'}
     aria-label={`Notificações: ${count} pendentes ou não lidas`}
     aria-expanded={open}
     onClick={()=>{setOpen(true);void load();}}
     style={{...btn,position:'relative',background:'#1e293b',color:'#fff',fontSize:18}}
   >
     <Bell size={27} weight="regular"/>
     <span style={{position:'absolute',top:-7,right:-7,background:error?'#b45309':'#dc2626',color:'#fff',borderRadius:20,minWidth:19,fontSize:12,padding:'2px 4px'}}>{error?'!':count>99?'99+':count}</span>
   </button>
   {modal}
 </>;
}
