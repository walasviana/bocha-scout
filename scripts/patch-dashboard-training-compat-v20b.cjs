...
const selectStart = src.lastIndexOf('<select', opponentSelectPos);
if (selectStart < newViewStart) throw new Error('v20b: início do select do adversário não encontrado');

const branchStart = src.lastIndexOf('{gameType === "Individual" ? (', selectStart);
if (branchStart < newViewStart) throw new Error('v20b: ramo Individual do adversário não encontrado');

// Substitui somente a abertura do ramo/campo e preserva o <select> completo.
// Assim não cortamos tags internas do label estilizado criado por patches anteriores.
const normalizedOpen = '{gameType === "Individual" ? (\n                  <Field label="Adversário cadastrado">\n                    ';
src = src.slice(0, branchStart) + normalizedOpen + src.slice(selectStart);
